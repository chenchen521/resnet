...
class ResNet(object):
  ...
  # 残差单元模块
  def _residual(self, x, in_filter, out_filter, stride, activate_before_residual=False):  #(x,32,32,[1,1,1,1],False)
    ...
    with tf.variable_scope('sub1'):
      ...
      x = self._conv('conv1', x, 3, in_filter, out_filter, stride)  #(3,3,16,16,16,[1,1,1,1])
    ...
    with tf.variable_scope('sub2'):
      ...
      x = self._conv('conv2', x, 3, out_filter, out_filter, [1, 1, 1, 1])  #(3,3,16,16,16,[1,1,1,1])
  ...
  def _bottleneck_residual(self, x, in_filter, out_filter, stride,
                           activate_before_residual=False):        #(16, 32, [1,1,1,1])
    # 是否前置激活(取残差直连之前进行BN和ReLU）
    if activate_before_residual:
